...
class Iterable {
  ...
  async scopeSelector (selector) {
    ...
    // Components have selectors that can match deeper elements. In order to
    // avoid going too deep when assigning children to multi-field items, we
    // store the depth in a data-attribute so we can filter out extra matches.
    // @see Component.setMultiFieldValues()
    // @see Step.getMultiFieldNestedContainerPropIndexes()
    // Because this happens during the init phase, elements will be processed
    // in the order from less deep to deepest, so if current match already has
    // been assigned a depth, if current depth is higher, we override the
    // previous value.
    // The selector is already scoped, so there won't be lower depths
    // incorrectly overridden (as long as the selector specificity excludes
    // elements outs of scope).
    if (this.constructor.name !== 'Step') {
      /* istanbul ignore next */
      await dom.evaluate(
        this.extractor.pageWorker.page,
